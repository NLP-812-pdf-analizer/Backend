...
def extract_graphs_from_text(text_path: str, model_dir: str, output_dir: str = "."):
    """
    Основная функция для извлечения графов из текста
    ...
    Returns:
        None (сохраняет два JSON файла с графами)
    """
    # Загрузка текста
...
def extract_graphs_from_text(text_content: str, model_dir: str):
    """
    Основная функция для извлечения графов из текста
    
    Args:
        text_content (str): Содержимое текстового файла
        model_dir (str): Путь к папке с моделью NER
    
    Returns:
        tuple: Две JSON-строки (функциональный граф, иерархический граф)
    """
    # Используем переданный текст напрямую
    text = text_content
    ...
    functional = json.dumps(
        {"graph": [{"source": rel[0], "relation": rel[1], "target": rel[2]} for rel in func_graph]},
        indent=2,
        ensure_ascii=False
    )
    
    hierarchical = json.dumps(
        {"graph": [{"source": rel[0], "relation": rel[1], "target": rel[2]} for rel in hier_graph]},
        indent=2,
        ensure_ascii=False
    )
...
def api_extract_graphs_from_pdf(pdf_path: str, model_path: str = None):
    ...
    functional_json, hierarchical_json = extract_graphs_from_text(clean_text, model_path)
...
if __name__ == "__main__":
    ...
    extract_graphs_from_text(args.text, args.model, args.output)
